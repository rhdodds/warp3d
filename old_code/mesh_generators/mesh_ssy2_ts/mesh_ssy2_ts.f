...
c
       do i=1,2*mteta-1
	num_elem1 = num_elem1 + 1
	do j=1,4
	elem1(j,num_elem1)   = quad_surfe(j,i,k+1)
	elem1(j+4,num_elem1) = quad_surfe(j,i,k)
	end do
       end do
c
c	for configuration #2
c
       do i=2*mteta,num_quads
	num_elem2 = num_elem2 + 1
	do j=1,4
	elem2(j,num_elem2)   = quad_surfe(j,i,k+1)
	elem2(j+4,num_elem2) = quad_surfe(j,i,k)
	end do
       end do
c
c
...
c
      call  output_warp(nodes, coords, elem1, elem2, quad_surfn,
     &                  quad_coord,
...
c
      ...
      subroutine output_warp(nodes, coords, elem1,elem2, quad_surfn,
     &                       quad_coord,
     &                       num_nod, num_elem, max_surfn,
     &                       num_elem1, num_elem2,
...
c
	...
	logical kt_fields
	data kt_fields /.true./
	dimension xyrt(4,max_bc), uv_k(2,max_bc), uv_t(2,max_bc)
...
c
      ...
      do i=1,num_elem1
      write(1,1100) i,(elem1(k,i),k=1,8)
      end do
      do i=1,num_elem2
      write(1,1100) i+num_elem1,(elem2(k,i),k=1,8)
      end do
      close(1)
c
c
...
c
      appl_t=0.0
      if(t_ratio.ne.0.0) then
       appl_t = t_ratio * sig0 
      end if
c
      if(kt_fields) then
      incs_t=30
      call get_kt_field(xyrt, uv_k, uv_t, num_uvc,E,nu,
     &                 appl_J, appl_K, ple,
     &                 appl_t,incs_t)
      else
      call get_k_field(xyrt, uv_k, uv_t, num_uvc,E,nu,
     &                 appl_J, appl_K, ple,
     &                 appl_t)
      end if
c
c
      if(t_ratio.ne.0.0) then
       open(1,file=warp_const_t)
      do i=1, num_uvc
       if(uv_t(2,i).ne.0.0) then
       write(1,21) node_uvc(i),uv_t(1,i),uv_t(2,i)
       else
       write(1,24) node_uvc(i),uv_t(1,i)
       end if
      end do
c
      close(1)
...
c
c      iring1 = 0.75*dble(num_dr)
c      iring2 = 0.9*dble(num_dr)
       iring1 = 25
       iring2 = 55
c
...
c
	istring = istring - 1
       ...
       write(1,10)
c
c	Elements configurations
c
c		Config-1
c
       write(1,109)
       write(1,110) 1,num_elem1
       write(1,131) 
       write(1,10)
c
c		Config-2
c
       write(1,139)
       write(1,110) num_elem1+1, num_elem
       write(1,111) 
       write(1,10)
c
c               Incidences
...
c
        ...
        nblock2=128
c
	iblock1 = dble(num_elem1)/nblock1
	do i=1,iblock1
	write(1,1200) i,nblock1,1+(i-1)*nblock1
	end do
	ilast=nblock1*iblock1
	if(ilast.lt.num_elem1) then
	write(1,1200) iblock1+1,num_elem1-ilast,1+ilast
	iblock1 = iblock1 + 1
	end if
c
c
	iblock2 = dble(num_elem2)/nblock2
	do i=1,iblock2
	write(1,1200) i+iblock1,nblock2,num_elem1+1+(i-1)*nblock2
	end do
	ilast=nblock2*iblock2
	if(ilast.lt.num_elem2) then
	write(1,1200) iblock1+iblock2+1,num_elem2-ilast,
     &                num_elem1+1+ilast
	end if
c
c
c
c
          if(t_ratio.ne.0..and.kt_fields) then
c
        write(1,10)
        write(1,114)
       write(1,10)
       write(1,8)
        dumc1 =  dumc2 (1:18+istring) // ".const0' " 
        write(1,'(a)') dumc1
        dumc1 =  dumc2 (1:18+istring) // ".const_t' " 
        write(1,'(a)') dumc1
       write(1,10)
       write(1,9)
        write(1,10)
...
c
        ...
        write(1,718) incs_t
        write(1,10) 
716	format('c',8x,'Applied T/Sig0 ratio is ....',f8.2)
717	format('c',8x,'K and T-tress Fields are applied together')
718	format('c',8x,'up to ',i3,' steps.  Then K-field only')
c
	write(1,10)
	write(1,10)
c
...
c
	...
	iold = 0
       do i=num_output, incs_t, num_output
	write(1,10)
        write(1,303) iold+1,i
        write(1,'(a)') dumc1
        write(1,'(a)') dumc3
	write(1,10)
	iold = i
...
c
       ...
       write(1,8)
        dumc1 =  dumc2 (1:18+istring) // ".const0' " 
        write(1,'(a)') dumc1
        dumc1 =  dumc2 (1:18+istring) // ".const_k' " 
        write(1,'(a)') dumc1
       write(1,10)
       write(1,9)
        write(1,10)
...
c
        ...
        write(1,904)
        write(1,905)
	write(1,907) incs_t+1,num_steps
        write(1,10)
        write(1,10)
        write(1,501)
        write(1,502)
...
c
        ...
        dumc3 =  dumc2 (1:18+istring) // ".domain' " 
	iold = incs_t 
       do i=incs_t+num_output, num_steps, num_output
	write(1,10)
...
c
	if(t_ratio.ne.0.0.and..not.kt_fields) then
c
        write(1,10)
        write(1,114)
       write(1,10)
...
c
        ...
        write(1,16) t_ratio
        write(1,10) 
16	format('c',8x,'Applied T/Sig0 ratio is ....',f8.2)
c
	write(1,10)
	write(1,10)
c
...
38        format('    q-values automatic rings ',i3,' - ',i3)
c 38        format('    q-values automatic rings 50-70 ')
39        format('    function type d')
...
103    format('    properties  mises  e ',f6.0,'  nu ',f3.1,
     &        '  yld_pt ',f9.4,'  n_power ',f5.2,',')
104    format('                rho 0.0')
105    format('number of nodes ',i8)
106    format('number of elements ',i8)
107    format('coordinates')
108    format('elements')
109    format('c   for config number   1')
139    format('c   for config number   2')
110    format(14x,i3,' - ',i5,' type l3disop nonlinear material steel,')
111    format(23x,'order 2x2x2 bbar center_output short')
131    format(23x,'order 2x2x2 no_bbar center_output short')
...
c
c
601     format ('loading unit_t')
602     format (' nodal_loads')
603     format ('    1  force_y 0.0')
c
604     format(' loading loadt')
605     format('  nonlinear')
606     format('    step 1  unit_t 1.0')
608     format('    step 1 -',i3,' unit_t 1.0')
607     format('compute displacements for loading loadt steps',
     &          ' 1 ')
c
c
c
901     format ('loading unit_j')
902     format (' nodal_loads')
903     format ('    1  force_y 0.0')
c
904     format(' loading load1')
905     format('  nonlinear')
906     format('    step 2 -',i3,' unit_j 1.0')
907     format('    step ',i3,' -',i3,' unit_j 1.0')
c
c
207     format(' dynamic analysis parameters')
208     format('  solution technique direct sparse sgi')
...
713     format('  minimum iterations 2')
213     format('  maximum iterations 10')
214     format('  convergence test norm res tol .0005 max res tol 0.05')  
...
220     format('  predict on')
c
301     format('compute displacements for loading load1 steps ',
     &          i3,' - ',i3)
302     format('stop')
303     format('compute displacements for loading loadt steps ',
     &          i3,' - ',i3)
c
c
...
c
	nline1 = (dble(num_nod)-0.1)/dble(nd_line)
...
c
	...
	return
       ...
       subroutine get_k_field(xyrt, uv_k, uv_t, num_uvc, E, nu,
     ...
     &                        applied_t)
       ...
       integer ple
       parameter (pi=3.14159265358979)
       dimension xyrt(4,*), uv_k(2,*), uv_t(2,*)
c
        G = E/(2.0*(1.0+nu))
c
	if(ple.eq.1) then
...
c            T-stress
c
         Tx = applied_t*(1.-nu*nu)/E
         Ty = -1.0*applied_t*nu*(1.+nu)/E
...
c
	 ...
	 kappa = (3.0 - nu) / ( 1.0 + nu )
c
c	NOTE:  T-stress for plane-stress need to be added here
c
        Tx = 0.0
        Ty = 0.0
c
...
c
	 ...
	 return
       ...
       subroutine get_kt_field(xyrt, uv_k, uv_t, num_uvc, E, nu,
     &                        applied_J, applied_K, ple,
     &                        applied_t,incs)
c
c	Add K filed and T-stress fields together for incs
c       and then continue the rest of the analysis with K-field
c       only.
c
       implicit real*8(a-h,o-z)
       real*8 nu, kappa
...
c            T-stress
c
         Tx =( applied_t*(1.-nu*nu)/E ) / float(incs)
         Ty =( -1.0*applied_t*nu*(1.+nu)/E ) / float(incs)
...
c
	...
	do k=1,num_r
	 Sum=1.0
	 do i=2,mr(k)
	  Sum = Sum + biase(k)**( dble(i-1) )
         end do
	 a1 = R1(k) / Sum
	 do i=1,mr(k)
	  ii= ii + 1
	  an = a1 * biase(k)**( dble(i-1))
	  dr(ii) = dr(ii-1) + an
	 end do
	end do
c
	Rt = dr(ii)
c
	dteta_r=(pi/2.0)/ dble(mteta_r)
	num_nod = 0
	teta_r=0.0
	do i=1,mteta_r + 1
	 teta_r = (dble(i)-1.0)*dteta_r
	 num_nod = num_nod + 1
...
c
	dl= ( dr(2)+0.5*l2 )/(dble(mteta_l) - 1.0 )
	do i=1,mteta_l - 1
	 num_nod = num_nod + 1
	 quad_surfn(num_nod,ilr) = num_nod + num_nodg
	 quad_coord(1,num_nod,ilr) = x0 - dl*dble(i)
...
c
	do i=1,mteta_l - 2
     ...
     &           ellipse_y( xl, dl*(dble(mteta_l)-1.0), dr(2))
	   yl =  ellipse_y( xl, dl*(dble(mteta_l)-1.0), dr(2))
	 quad_coord(3,num_nod,ilr) = z
	end do
c
c
...
c
	...
	alpha = dAtan( dr(1)/xl )
	teta_l = (PI/2.0) - alpha
	dteta_l = teta_l / dble( mteta_l )
c
	do i=1,mteta_l -1
	 num_nod = num_nod + 1
	 teta_l = dble(i)*dteta_l
	 quad_surfn(num_nod,ilr) = num_nod + num_nodg
...
c
      call inp_back(inpf,ifirst,ilast,ierr)
      read(inpf,*) mteta_r, mteta_l
      mteta = mteta_r + mteta_l
c
...
c
       ...
       warp_const0 = dumc1 (ifirst:ilast) // '.const0'
       warp_const_k= dumc1 (ifirst:ilast) // '.const_k'
       warp_const_t= dumc1 (ifirst:ilast) // '.const_t'
       warp_out    = dumc1 (ifirst:ilast) // '.output'
       warp_domain = dumc1 (ifirst:ilast) // '.domain'
       aba_nsets   = dumc1 (ifirst:ilast) // '.abaqus'
c
...
c
       ...
       inquire (file = warp_const0 , exist = file_exist )
       inquire (file = warp_const_k , exist = file_exist )
       inquire (file = warp_const_t , exist = file_exist )
       inquire (file = warp_out   , exist = file_exist )
       inquire (file = warp_domain, exist = file_exist )
       inquire (file = aba_nsets  , exist = file_exist )
       if(file_exist) then
       write(*,*)
...
c
c	Input: E, nu, sig0, n_power
c	Where:
c                E       -- Young's Modulus
c                nu      -- Piosson's Ratio
c                sig0    -- Yield stress
c                n_power -- Power Degree
c
       call inp_back(inpf,ifirst,ilast,ierr)
       read(inpf,*)  E, nu, sig0, n_power
c
c	Input: appl_J, T_ratio
c	Where:
c                appl_J -- Increment of J if the material were elastic
c                appl_K -- will be calculated from appl_J for pl-e or pl-s
c                T_ratio -- T/sig0 ( T-stress )  It is applied in one step
c
       call inp_back(inpf,ifirst,ilast,ierr)
       read(inpf,*)  appl_J, T_ratio
c
      return
      end
