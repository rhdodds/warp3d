...
c                                      jacflg[1]:  0 numeric; 1 user supplied; 2
c                                                  3: user supplied banded      
c                                      jacflg[2]: number of sub diagonals or -1 
c                                      jacflg[3]: number of super diagonals or -
c                                      jacflg[4]: 1 if adjusting step allowed wh
...
c     In       qrwork  Real(*)         workspace for Lapack QR routines (call li
...
c                                       outopt(2)                               
c                                         0 do not check any user supplied jacob
c                                         1 check user supplied jacobian if supp
...
c                                             5  singular/ill-conditioned jacobi
...
c                                            -3  cannot check user supplied jaco
c                                                                               
...
c     Inout    scalex  Real(*)         scaling factors x()                      
c     Inout    xscalm  integer         0 for fixed scaling, 1 for automatic scal
...
c                                                                               
      integer :: length                                                         
      complex(kind=real64), dimension (solve_work%props%num_hard) :: x2i !  was 
      double precision, dimension(solve_work%props%num_hard) :: zero2 ! was dime
      if( solve_work%solvfnc == 1 ) then                                        
        zero2 = zero                                                            
...
c     Arguments                                                                 
c                                                                               
c       Inout  R      Real(ldr,*)     upper triangular matrix R from QR (unalter
c                                     strict lower triangle contains            
c                                        transposed strict upper triangle of the
...
c                                                                               
c       Out    d      Real(*)         vector with step with norm very close to d
...
c                                                                               
c     Algorithm comes from More: The Levenberg-Marquardt algorithm, Implementati
c     Lecture Notes in Mathematics, 1978, no. 630.                              
c     uses liqrev (in file liqrev.f) which is based on Nocedal's method (see com
c-----------------------------------------------------------------------------  
...
c     Description                                                               
c                                                                               
c     Given an n by n upper triangular matrix R, a diagonal matrix D with positi
...
c     The first is to provide a method of slightly modifying a singular or ill-c
c     The second is for calculating a least squares solution to the above proble
c     the context of e.g. a Levenberg-Marquardt algorithm combined with a More-H
c     to determine a value of D (diagonal mu) such that x has a predetermined 2-
c                                                                               
c     The routine could also be used when the matrix R from the QR decomposition
c     is ill-conditioned (or singular). Then it is difficult to calculate a Newt
c     accurately (Dennis and Schnabel). D&S advise perturbing trans(J)*J with a 
c     diagonal matrix.                                                          
c                                                                               
c     The idea is  to solve (J^T * J + mu*I)x=b where mu is a small positive num
c     Calculation of mu must be done in the calling routine.                    
c     Using a QR decomposition of J solving this system                         
c     is equivalent solving (R^T*R + mu*I)x=b, where R comes from the QR decompo
c     Solving this system is equivalent to solving the above least squares probl
c     elements of the matrix D set to sqrt(mu) which should be done in the calli
c                                                                               
...
c     Nocedal and Wright, 2006, Numerical Optimization, Springer, ISBN 978-0-387
...
c           rows of R have been copied into the columns of R initially (see abov
...
c     Arguments                                                                 
c                                                                               
c     Out      rjac    Real(ldr,*)     jacobian (n columns) and used for storing
...
c                                          increased if needed after first itera
...
c     Arguments                                                                 
c                                                                               
c     Inout    rjac    Real(ldr,*)     jacobian matrix at current iterate; on re
c     Inout    r       Real(ldr,*)     jacobian matrix at current iterate; on re
c     In       ldr     Integer         leading dimension of rjac                
c     In       fn      Real(*)         function values at current iterate       
c     In       n       Integer         dimension of problem                     
c     In       cndtol  Real            tolerance of test for ill conditioning   
c     In       stepadj Logical         allow adjusting step for singular/illcond
c     Inout    qraux   Real(*)         QR info from liqrfa (calling Lapack dgeqr
c     Out      dn      Real(*)         Newton direction                         
c     Out      qtf     Real(*)         trans(Q)*f()                             
c     Out      ierr    Integer         0 indicating Jacobian not ill-conditioned
...
c     Out      rcond   Real            inverse condition of upper triangular R o
c     Wk       rcdwrk  Real(*)         workspace                                
c     Wk       icdwrk  Integer(*)      workspace                                
c     In       qrwork  Real(*)         workspace for Lapack QR routines (call li
c     In       qrwsiz  Integer         size of qrwork                           
...
c     Inout    rjac    Real(ldr,*)     jacobian (n columns)(compact QR decomposi
...
c     evaluate user supplied or finite difference jacobian and check user suppli
...
c     In       priter  Integer           >0 unit if intermediate steps to be pri
c                                        -1 if no printing                      
c                                                                               
c-------------------------------------------------------------------------      
...
c                      Higham: Accuracy and Stability of Numerical Algorithms, s
c                      Actually solving 3*a*x^2+2*b*x+c=0 ==> (3/2)*a*x^2+b*x+(c
...
c     In       ldr     Integer         leading dimension of R                   
c     In       n       Integer         size of problem                          
c     Out      sdiag   Real(*)         diagonal of LM lower triangular modified 
c     In       qtf     Real(*)         trans(Q)*f(xc)                           
c     In       dn      Real(*)         current newton step                      
c     Out      dnlen   Real            length dn()                              
...
c        calculate LM step                                                      
     ...
     *                delta,mu, d, work)                                        
c        change sign of step d (limhpar solves for trans(R)*R+mu*I)=qtf instead 
...
c     In       ierr    Integer         return code of cndjac (condition estimati
c                                                                               
c     Out      termcd  Integer         termination code                         
...
c-------------------------------------------------------------------------      
c                                                                               
c     Check the user supplied jacobian against its finite difference approximati
c                                                                               
c     Arguments                                                                 
c                                                                               
...
c-------------------------------------------------------------------------      
c                                                                               
c     Check the user supplied jacobian against its complex difference approximat
c                                                                               
c     Arguments                                                                 
c                                                                               
c     In       A       Real(lda,*)     user supplied jacobian                   
...
c     the analytic one                                                          
      ...
      t2 = sqrt( 1.0d0 / Rten**ndigit )                                         
      p = max( t1, t2 )                                                         
c      p = (Rten**4)*(max(Rten**(-ndigit),epsm)) ! this one did not work, gave s
      tol    = epsm**Rquart                                                     
...
c                                      jacflg[2]: number of sub diagonals or -1 
c                                      jacflg[3]: number of super diagonals or -
c                                      jacflg[4]: 1 if adjusting jacobian allowe
c                                                   singular or illconditioned  
c     In       scalex  Real(*)         scaling vector for x()                   
c     Wk       fz      Real(*)         workspace                                
c     Wk       wa      Real(*)         workspace                                
...
c            fdjac0 for why                                                     
c            doing this ensures that results for fdjac2 and fdjac0 will be ident
...
c     In       ldr     Integer         leading dimension of rjac                
c     In       cndtol  Real            tolerance of test for ill conditioning   
c                                       when rcond <= cndtol then ierr is set to
