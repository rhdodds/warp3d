...
c     ****************************************************************
c     *                                                              *
c     *          subroutine set_mm05_props_for_block                 *
c     *                                                              *
c     *                       written by : rhd                       *
c     *                                                              *
c     *                   last modified : 08/7/2011                  *
c     *                                                              *
c     *     set up the temperature dependent properties for the      *
c     *     cyclic plasticity model (mm05). at this time, only       *
c     *     the generalized plasticity option has temperature        *
c     *     dependent properties                                     *
c     *                                                              *
c     ****************************************************************
c
      subroutine set_mm05_props_for_block(
     &  span, nu_block, e_block, mm05_props, nonlin_hard,
     &  generalized_pl, segmental, felem, etype, int_order,
     &  gpn, nnodel, temps_node_to_process, temps_node_blk,
     &  iout )
c
      use segmental_curves, only :
     ...
     &      seg_curves_gp_sigma_0, seg_curves_gp_delta_u,
     &      seg_curves_gp_beta_u, seg_curves_gp_sigma_0,
     &      seg_curves_gp_h_u
c
      implicit integer (a-z)
$add common.main
...
#dbl      double precision
#sgl      real
     &   e_block(*), nu_block(*), mm05_props(mxvl,*),
     &   temps_node_blk(mxvl,*), gp_h_u, gp_sig_0,
     &   gp_beta_u, gp_delta_u
      logical segmental, temps_node_to_process, nonlin_hard,
     &        generalized_pl
c
c                local declarations
c
...
#sgl      real
     ...
     &   point_temp, linear_interpolate
      data local_debug, zero / .false., 0.0 /
c
c                we can only have temperature dependent
c                proeprties for mm05 if (1) the material definition
c                has segmental stress-strain curves [actual curves
c                are not defined, just the temperature dependence
c                of the properties], (2) the generalized
c                plasticity option is in effect.
c                if not, the current nu_block,
c                e_block and mm05_props values
c                are constant for all elements in the block.
c
      if ( local_debug ) then
        write(iout,*) '.... mm05_props_for_block'
        write(*,*) '... segmental, generalized_pl: ',
     &           segmental, generalized_pl
      end if
      if ( .not. segmental ) return
      if ( .not. generalized_pl ) return
c
c                the material for elements in the block is defined
c                by one or more temperature dependent sets (called curves)
c                of properties. if they are not
c                temperature dependent, ther pre-loaded values are correct
c                and constant for all elements in the block.
c
      curve_set         = iprops(21,felem)
      first_curve       = seg_curve_table(2,curve_set)
      num_curves_in_set = seg_curve_table(1,curve_set)
c
      if ( seg_curves_type(first_curve) .ne. 1 ) return
c
c                properties are temperature dependent.
c                extract values and corresponding temperatures
c                from user-defined curves (in order of increasing temp).
c                this is done only for the first gauss point of the
c                element block and saved for reuse during processing
c                of subsequent gauss points for element block.
c                all elements of the block must use the same set of
c                user-defined, temperature dependent curves.
c
      if ( gpn .eq. 1 ) then
         if ( allocated( curve_temps ) )
     &     deallocate( curve_temps, curve_e_values,
     ...
     &      curve_temps(ksize), curve_e_values(ksize),
     &      curve_nu_values(ksize), curve_alpha_values(ksize),
     &      curve_gp_sig_0_values(ksize), curve_gp_h_u_values(ksize),
     &      curve_gp_beta_u_values(ksize),
     &      curve_gp_delta_u_values(ksize) )
         do i = 1, num_curves_in_set
          curve_no                   = seg_curve_table(i+1,curve_set)
          curve_temps(i)             = seg_curves_value(curve_no)
...
c
      if ( local_debug ) then
        write(iout,9000) felem, curve_set, first_curve,
     &                   num_curves_in_set
      end if
c
c                the material for elements in the block is described by
c                a set of temperature dependent curves.
c
c                get the temperature at this gauss point for each
c                element in the block. interpolate from node
c                temperatures unless they are all zero.
...
c                get property values for each element in the block
c                at this gauss point.
c
c                gp_sig_0 computed but not actually used now
c                by cnst for generalized plasticity
c
      do ielem = 1, span
        point_temp     = gp_temps(ielem)
        e_block(ielem)  = linear_interpolate( point_temp,
     ...
     &                        num_curves_in_set, curve_temps,
     &                        curve_nu_values )
        gp_sig_0        = linear_interpolate( point_temp,
     &                        num_curves_in_set, curve_temps,
     &                        curve_gp_sig_0_values )
        gp_h_u          = linear_interpolate( point_temp,
     &                        num_curves_in_set, curve_temps,
     &                        curve_gp_h_u_values )
        gp_beta_u       = linear_interpolate( point_temp,
     &                        num_curves_in_set, curve_temps,
     &                        curve_gp_beta_u_values )
        gp_delta_u      = linear_interpolate( point_temp,
     &                        num_curves_in_set, curve_temps,
     &                        curve_gp_delta_u_values )
        mm05_props(ielem,1) =  gp_h_u
        mm05_props(ielem,3) =  gp_beta_u
        mm05_props(ielem,5) =  gp_delta_u
      end do
c
      if ( local_debug ) then
       write(iout,9100) gpn
       do ielem = 1, span
         write(iout,9200) ielem, gp_temps(ielem), e_block(ielem),
     &                 nu_block(ielem), mm05_props(ielem,1),
     &                 mm05_props(ielem,3), mm05_props(ielem,5)
       end do
      end if
c
      return
c
 9000 format('>> inside set_mm05_props_for_block.',
     & ' material is temperature dependent.',
     & /,4x,'felem, curve_set, first_curve, num_curves_in_set: ',i8,
     & 3i3 )
 9100 format('>> mm05_props interpolated at elements for block, gpn:',i2,
     & /,'   rel. elem      temp            e          nu',
     &   '       h_u    beta_u    delta_u')
 9200 format(10x,i4, f12.4, f12.2, f10.4, 3f10.2)
c
      end
