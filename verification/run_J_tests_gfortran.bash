...
function testE {     # MPI Linux only, w/ Intel fortran
...
if [ "$GFORTRAN" = 'yes' ]; then
       printf "\n\t>> Cannot run MPI jobs on Linux w/ gfortran build\n\n"
       return
fi
...
if [ "${machine:0:6}" = 'Darwin' ]; then
    ...
    name="$WARP3D_HOME/warp3d_script_mac_os_x_gfortran"
    if [ ! -f "$name" ]
     then
       printf "\n>> Expecting to find a warp3d_script_mac_os_x_gfortran file that runs WARP3D\n"
...
if [ "${machine:0:5}" = 'Linux' ]; then
    ...
    name="$WARP3D_HOME/warp3d_script_linux_gfortran"
    GFORTRAN="yes"
    INTEL_FORTRAN="no"
    if [ ! -f "$name" ]
     then
       printf "\n>> Expecting to find the bash shell script:\n"
       printf   "   warp3d_script_linux_gfortran in the WARP3D_HOME directory.\n"
