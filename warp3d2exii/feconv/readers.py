...
#		Setup the results reader
	...
	def nset_iterator(self):
		...
		return (("All Nodes", range(self.num_nodes)),)
	...
	def node_iterator(self):
		...
		class PatranNodeIt(PatranNeutralIt):
			...
			def next(self):
				...
				packet, data = super(PatranNodeIt, self).next()
				while packet[0] != 1:
					packet, data = super(PatranNodeIt, self).next()
	...
	def elem_iterator(self):
		...
		class PatranElemIt(PatranNeutralIt):
			...
			def next(self):
				...
				packet, data = super(PatranElemIt, self).next()
				while packet[0] != 2:
					packet, data = super(PatranElemIt, self).next()
		...
		class ExodusNodeIt(object):
			...
			def next(self):
				if self.n == self.exo.num_nodes:
					...
					for i,name in zip(range(self.exo.dim), cnames):
