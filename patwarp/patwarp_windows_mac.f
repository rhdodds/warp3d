...
c
      call trntran
c
c            reorder elements in to blocks to warp3d input file.
c
      call trnler
c
...
c
c
c           The Windows and Mac OS x versions of WARP3D do not support MPI
c           based parallel execution - only threads based execution.
c           This version of patwarp started with the Linux version
c           has has the MPI features disabled (e.g., the Metis
c           package for domain decomposition is a dummy
c           routine here.
c
c           The threads only version of WARP3D supports the
c           Intel/Pardiso sparse (direct and iterative) solver
c           and the element-by-element
c           preconditioned conjugate gradient solver.
c
c           Vectorized blocking for old EBE solver
c           removed from options
c           Try to use the automatic blocking provided by
c           WARP3D
c
c           if the Windows or Mac OS X version of WARP3D ever supports MPI,
c           delete this program and use the Linux version fo patwarp.
c
c
      q_num = 2
      ques_ans = 'n'   ! MPI not available on Windows, OS X
      call process_answer(q_num,ques_ans,default_ans)
c
      q_num = 3   ! blocking. < 0 means vectorized
      block_size = 128
      block_method = 1
...
c
 10   continue
c
c                      The only case processed in remainder of code
c                      here is threads only parallel on Windows and
c                      Macs for vectorized blocking to support the EBE
c                      pre-conditioner.
c
c                      Elements will necessarily be re-ordered in this
c                      process otherwise blocks would likely be very
c                      small.
c
c                      If block size is positive at this point, we have
c                      an inconsistency in the input/checking system.
c
c                      Set logical flags here so the more general code
c                      taken from Linux (MPI+threads) version of
c                      patwarp will work for this case.
c
      parallel = .false.
      scalar   = .false.
      numprocs = 1
...
c                       elements.
c
c                       if this is parallel, first determine the element
...
c
 ...
 9100 format (/,7x,' >> Notes:',/,
     &        7x,'       >> Non-vectorized blocking specified. ',/,
...
c     *                       written by : bh                        *
c     *                                                              *
c     *          last modified by ag : 02/15/94                      *
c     *                                                              *
...
c
      call stripf(resfile)
      if (resfile(1:1).eq.' ') resfile = 'pat_info.els'
c
