...
c                                                                               
      subroutine determine_local_sparsity()                                     
            use global_data ! formerly common.main                              
            use local_stiffness_mod                                             
            use distributed_stiffness_data                                      
            use performance_data                                                
...
c                                                                               
      subroutine assemble_sparsity                                              
            use global_data ! fomerly common.main                               
            use performance_data                                                
            use distributed_stiffness_data                                      
            use local_stiffness_mod                                             
            use vararray_mod                                                    
...
c           Loop on your final equations, receiving and processing as appropriat
            do eq=1,local_k%local_n                                             
                  geq = final_my_eqns(eq)                                       
                  leq = my_eqns_inv(geq)                                        
...
c    Step 1 - transverse col_indices figuring out how many LT element there will
      do i=1,ncoeff                                                             
            work(k_indices(i))=work(k_indices(i))+1                             
      end do                                                                    
c                                                                               
c    Please remember that the real nnz in each row LT is actually -1 this (diago
      do i=1,neq                                                                
            work(i) = work(i)-1                                                 
      end do                                                                    
c                                                                               
...
c                                                                               
c    Step 4 - adjust row_ptrs to the new correct amounts (each row gets shifted 
      offsetv = ltsum                                                           
      do i=neq+1,2,-1                                                           
...
c                                                                               
c    Step 5 - now transverse our entire UT minus diagonal and insert the transpo
