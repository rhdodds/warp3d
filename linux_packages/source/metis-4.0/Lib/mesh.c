...
* bricks.
******************************************************************************/
void METIS_MeshToDual(int *ne, int *nn, idxtype *elmnts, idxtype *etype, int *numflag, 
                      idxtype *dxadj, idxtype *dadjncy)
{
  ...
  count = 0;
  for (i=0; i<*ne; i++) 
     count += esizes[etype[i]];

  if (*numflag == 1)
    ChangeMesh2CNumbering(count, elmnts);

  GENDUALMETIS(*ne, *nn, count, etype, elmnts, dxadj, dadjncy);

  if (*numflag == 1)
    ChangeMesh2FNumbering(count, elmnts, *ne, dxadj, dadjncy);
...
******************************************************************************/
void GENDUALMETIS(int nelmnts, int nvtxs, int count, idxtype *etype, idxtype *elmnts, idxtype *dxadj, idxtype *dadjncy)
{
   int i, j, jj, k, kk, kkk, l, m, n, nedges, mask, cnt;
   idxtype *nptr, *nind;
